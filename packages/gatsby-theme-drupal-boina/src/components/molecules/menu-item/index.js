...
export default class MenuItem extends Component {
  ...
  componentDidMount() {
    ...
    const parentSelected = _find(
      category.categoryPages,
      ({ node }) =>
        _has(location, 'pathname') &&
        node.path.alias === location.pathname.replace(/\/$/, '')
    )
  ...
  render() {
    const { category, location } = this.props
    const { node: firstContent = null } = _has(
      _first(category.categoryPages),
      'node'
    )
      ? _first(category.categoryPages)
      : {}
    ...
    return (
      <li
        ...
        className={`c-menu-item o-list-bare c-menu-item__parent ${
          parentSelected || this.state.parentSelected
            ? 'c-menu-item__parent--selected'
            : ''
        }`}
      >
        <span className="c-menu-item__link u-tiny-text u-border--bottom grid-x align-middle">
          {category.name}
        </span>
        <ul className="c-menu-item__submenu">
          {category.categoryPages.map(({ node }) => {
            const currentPageSelected =
              _has(location, 'pathname') &&
              location.pathname.replace(/\/$/, '') === node.path.alias
                ? 'c-menu-item__link--active'
                : ''
            return (
              <li key={node.id} className="o-list-bare u-border--bottom">
                <Link
                  className={`c-menu-item__link u-tiny-text ${currentPageSelected}`}
                  activeClassName="c-menu-item__link--active"
                  to={node.path.alias}
                >
                  {node.title}
                </Link>

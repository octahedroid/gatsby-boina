...
class Layout extends Component {
  ...
  handleScroll = (event) =>
    this.setState({
      scrolledMenu: event.srcElement.scrollingElement.scrollTop > 30,
    })
  handleResize = (event) => {
    const isMobile = window.innerWidth > this.state.mobileSize
    this.setState({
      ...
      <StaticQuery
        ...
        render={(data) => {
          const {
            ...
            field_footer,
            field_twitter_handle,
          } = data.allSiteSettingEntitySite.edges[0].node
          const postData = {
            ...
            <>
              ...
              <div className="c-layout">
                <Sidebar
                  ...
                  hideSidebar={this.handleShowSidebar}
                />
                <div className="cell medium-cell-block">
                  <Header
                    scrolled={this.state.scrolledMenu}
                    isMobile={this.state.isMobile}
                    siteTitle={field_name}
                    showSidebar={this.handleShowSidebar}
                    darkMenu={this.props.darkMenu}
                  />
                  <div className="c-layout__main cell medium-cell-block">
                    {children}
                  </div>
                  <Footer
                    showCta={this.props.showFooterCta}
                    copyContent={field_footer}
                  />
                </div>
              </div>
            </>
